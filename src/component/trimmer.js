 ...
 export default class Trimmer extends React.Component{
	    ...
	    filter1=()=>{
		 ...
		 filter3=()=>{
			 let dis=document.getElementById("filter13").style.display;
			 if(dis=="none")
				document.getElementById("filter13").style.display="block";
			else
				document.getElementById("filter13").style.display="none";
		 }
		 ...
		 filter_search=()=>{
			 ...
			 else if(checked=="philips"){
				 let content=[];
					for(let i=0;i<this.state.copy.length;i++)
					{
						if(this.state.copy[i].model==checked){
							content.push(this.state.copy[i]);
						}
					}
					this.setState({list:content});
					console.log(content);
			 }
			  else if(checked=="wahl"){
				 let content=[];
					for(let i=0;i<this.state.copy.length;i++)
					{
						if(this.state.copy[i].model==checked){
		...
		return(
				...
				<div id="holder">	
					<div id="filter" ><i class="fa fa-filter c6" aria-hidden="true"> Filter</i></div>
						...
						<div class="fil" >
							<div onClick={this.filter3}>By brands <i class="fa fa-caret-down " aria-hidden="true"></i></div>
							<div id="filter13">
									<div class="fil_content">< input class="radio" name="radio" type="radio" value="philips"/>philips</div>
									<div class="fil_content">< input class="radio" name="radio" type="radio" value="braun"/>braun</div>
									<div class="fil_content">< input class="radio" name="radio" type="radio" value="wahl"/>wahl</div>
							</div>
						</div>
					</div>
